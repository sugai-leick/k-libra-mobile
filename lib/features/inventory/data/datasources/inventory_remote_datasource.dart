...
class InventoryRemoteDatasourceImpl implements InventoryRemoteDatasource {
    ...
    try {
      ...
      if (response.data != null) {
        if (response.data is Map<String, dynamic>) {
          ...
          final hwList =
              map['hardware'] ?? map['hardwares'] ?? map['equipments'] ?? [];
          final csList =
              map['items'] ?? map['consumables'] ?? map['products'] ?? [];

          if (hwList is List) {
            results['hardware'] = hwList
                .map((e) => HardwareModel.fromJson(e))
                .toList();
          }
          if (csList is List) {
            results['consumables'] = csList
                .map((e) => ConsumableModel.fromJson(e))
                .toList();
          }
      ...
      throw Exception(
        'Falha ao obter estoque da API: ${e.response?.statusMessage ?? e.message}',
      );
    } catch (e) {
  ...
  Future<void> registerHardware(AddHardwareDto dto) async {
    ...
    try {
      ...
      throw Exception(
        'Falha ao adicionar hardware: ${e.response?.data?['message'] ?? e.message}',
      );
    } catch (e) {
  ...
  Future<void> registerTransaction(InventoryTransactionDto dto) async {
    try {
      ...
      throw Exception(
        'Falha ao lançar transação de estoque: ${e.response?.data?['message'] ?? e.message}',
      );
    } catch (e) {
